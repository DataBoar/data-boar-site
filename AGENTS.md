...
- **Site chrome sync:** `.announce` + `#site-nav` + `.nav-cta` + `<footer>` are identical on every page (Agendar only in `.announce`). Edit only `partials/site-chrome/`, then `python3 scripts/sync-site-chrome.py`. Guarded by `--check` in pre-commit / check-all / `tests/test_site_surface.py` + `tests/test_guardrails.py`.
